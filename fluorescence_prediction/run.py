...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--instruction", default="Detect organoid viability and explain the image evidence.")
...
def resolve_language(requested: str, instruction: str) -> str:
    if requested in {"en", "zh"}:
        return requested
    lowered = instruction.lower()
    if any(token in lowered for token in ("english", "in english", "not chinese")):
        return "en"
    if re.search(r"[\u3400-\u9fff]", instruction):
        return "zh"
    return "en"


def explain_sample(sample: dict, language: str) -> str:
    ...
    if language == "zh":
        ...
        notes.append(f"区域内部对比度为 {contrast:.3f}、边缘密度为 {edge:.3f}；这些是图像证据描述，不能单独证明生物学因果。")
    else:
        regularity = "relatively regular" if circularity >= 0.70 else "somewhat irregular"
        notes.append(f"The segmented region has a circularity of {circularity:.3f} and a bounding-box fill of {fill:.3f}; its outline is {regularity}.")
        notes.append(f"Internal contrast is {contrast:.3f} and edge density is {edge:.3f}. These are descriptions of image evidence and do not independently establish biological causality.")
    return " ".join(notes)


def analyze_one(index: int, path: Path, output_dir: Path, predictor: OrganoidPredictor, segmentor: OrganoidSegmentor, language: str) -> dict:
    started = time.perf_counter()
    source_bytes = path.read_bytes()
    segmented = segmentor.segment(source_bytes)
    prediction = predictor.predict(segmented.crop_jpeg, language=language)
    ...
    sample["explanation"] = explain_sample(sample, language)
...
def main() -> int:
    args = parse_args()
    language = resolve_language(args.language, args.instruction)
    started = time.perf_counter()
    ...
    for index, path in enumerate(paths, start=1):
        ...
        try:
            sample = analyze_one(index, path, output_dir, predictor, segmentor, language)
    ...
    report = reporter.generate(samples, failures, language=language)
    result = {
        ...
        "disclaimer": (
            "结果仅供科研分析参考；解释基于模型输出和可计算图像特征，不构成生物学因果证明。"
            if language == "zh"
            else "Results are for research use only. Interpretations are based on model outputs and measurable image features and do not establish biological causality."
        ),
    }
