...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Write a compact summary for the Yichao max/max B2F run.")
...
def main() -> int:
    ...
    projected_summary = read_json(args.projected_root / "manifests" / "summary.json")
    ...
    external_metrics = read_json(args.external_root / "evaluation" / "Data-Yichao-11_maxbf_maxfl_last_epoch_external_metrics.json")
    ...
    payload = {
        ...
        "paths": {
            "projected_root": str(args.projected_root),
            ...
            "external_panel": str(args.external_root / "evaluation" / "Data-Yichao-11_maxbf_maxfl_last_epoch_external_panel.png"),
        },
    }
    ...
    lines = [
        "# Yichao Max-Brightfield / Max-Fluorescence B2F Run",
        "",
        "Projection policy: max projection for brightfield and max projection for fluorescence.",
        ...
        "",
        "## Paths",
        f"- Projected instance root: `{args.projected_root}`",
        ...
        "",
        "## Dataset Counts",
        f"- Projected images: `{projected_summary.get('projected_image_count', 'pending')}`",
        f"- Projected instances: `{projected_summary.get('projected_instance_count', 'pending')}`",
        f"- Complete non-edge projected instances: `{projected_summary.get('complete_non_edge_instance_count', 'pending')}`",
    ...
    lines.extend(
        [
            ...
            f"- External Data-Yichao-11 panel: `{args.external_root / 'evaluation' / 'Data-Yichao-11_maxbf_maxfl_last_epoch_external_panel.png'}`",
        ]
    )
