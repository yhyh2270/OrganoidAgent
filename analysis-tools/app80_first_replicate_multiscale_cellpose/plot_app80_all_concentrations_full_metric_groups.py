...
def plot_group(rows: list[dict], out_dir: Path, group_name: str, specs: list[tuple[str, str, str | None]]) -> tuple[Path, Path]:
    ...
    handles = [
        Line2D([0], [0], color=CONCENTRATION_COLORS[c], lw=2.2, marker='o', markersize=4.8, label=CONCENTRATION_LABELS[c])
        for c in CONCENTRATION_ORDER if c in by_conc
    ]
    ...
    title_map = {
        'growth_full': 'App80 10x Across Concentrations: Growth Metrics (Full)',
        'fusion_core_full': 'App80 10x Across Concentrations: Fusion Metrics (Core)',
        'fusion_internal_edge_full': 'App80 10x Across Concentrations: Fusion Metrics (Internal Edge)',
        'differentiation_full': 'App80 10x Across Concentrations: Differentiation Metrics (Full)',
        'helpers_full': 'App80 10x Across Concentrations: Helper / Normalization Metrics',
    }
    ...
    png = out_dir / f'app80_all_concentrations_{group_name}.png'
    pdf = out_dir / f'app80_all_concentrations_{group_name}.pdf'
