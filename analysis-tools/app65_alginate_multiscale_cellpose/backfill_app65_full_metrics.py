...
DATE_RE = re.compile(r'^(\d{2})-(十一月|十二月)-(\d{4})$')
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument('--jobs', type=int, default=min(8, max(1, (os.cpu_count() or 1) // 2)))
    return p.parse_args()


def has_full_metric_set(data: dict) -> bool:
    required = (
        'sum_intensity',
...
def parse_date_dir(name: str) -> tuple[str, date]:
    ...
    date_label = f"{day:02d}-{'Nov' if month == 11 else 'Dec'}"
...
def compute_hybrid_signal(gray: np.ndarray) -> np.ndarray:
...
def load_or_rebuild_signal(signal_path: Path, source_tif: Path) -> np.ndarray:
    try:
        return load_gray_png(signal_path)
    except FileNotFoundError:
        gray = load_source_gray(source_tif)
        signal = compute_hybrid_signal(gray)
        signal_path.parent.mkdir(parents=True, exist_ok=True)
        if not cv2.imwrite(str(signal_path), signal):
            raise RuntimeError(f'Failed to rebuild signal: {signal_path}')
        return signal
...
def refresh_metric_json(metric_path: Path) -> dict:
    ...
    signal = load_or_rebuild_signal(signal_path, source_tif)
...
def load_metrics(out_dir: Path, *, skip_refresh: bool = False, jobs: int = 1) -> list[dict]:
    ...
    for metric_path in metric_paths:
        ...
        raw_by_path[metric_path] = data0
        _, dt = parse_date_dir(data0['date_key'])
        dates.append(dt)
    day0 = min(dates)
    if skip_refresh:
        pending_results: dict[Path, dict] = {}
    else:
        pending_paths = [metric_path for metric_path in metric_paths if not has_full_metric_set(raw_by_path[metric_path])]
        pending_results = {}
        if jobs > 1 and pending_paths:
            executor = cf.ProcessPoolExecutor(max_workers=jobs)
            data_iter = executor.map(refresh_metric_json, pending_paths, chunksize=8)
        elif pending_paths:
            data_iter = (refresh_metric_json(metric_path) for metric_path in pending_paths)
        else:
            data_iter = ()

    try:
        if not skip_refresh:
            for metric_path, data in zip(pending_paths, data_iter):
                pending_results[metric_path] = data
        for metric_path in metric_paths:
            data = raw_by_path[metric_path] if skip_refresh else pending_results.get(metric_path, raw_by_path[metric_path])
        ...
        if not skip_refresh and jobs > 1 and pending_paths:
            executor.shutdown(wait=True)
    metrics.sort(key=lambda r: (CONDITION_ORDER.index(r['condition']), r['relative_day'], natural_key(r['image_name'])))
    return metrics
...
def write_daily_summary(rows: list[dict], out_csv: Path) -> tuple[list[str], list[dict]]:
    ...
    for row in rows:
        key = (row['condition'], row['date_key'], row['date_label'], int(row['relative_day']))
...
def main() -> int:
    ...
    metrics = load_metrics(out_dir, skip_refresh=args.skip_refresh, jobs=max(1, int(args.jobs)))
    if not metrics:
