...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--statistics-figures", action="store_true")
    parser.add_argument("--comparison-gallery", action="store_true")
    parser.add_argument("--object-crops", action="store_true")
    parser.add_argument("--diagnostic-images", action="store_true")
    parser.add_argument("--results-csv", action="store_true")
    parser.add_argument(
        "--experiment",
...
def process_one(
    ...
    diagnostic_images: bool = False,
    object_crops: bool = False,
    ...
    required = [metric_path, stats_path, overlay_path]
    if diagnostic_images:
        required.extend(
            [mask_path, signal_path, support_path, rgb_path, cellpose_mask_path, cellpose_rgb_path, cellpose_overlay_path]
        )
    elif object_crops:
    ...
    if diagnostic_images or object_crops:
    ...
    metric = {
        ...
        "mask_path": str(mask_path.resolve()) if diagnostic_images else None,
        "working_mask_path": str(mask_path.resolve()) if object_crops and not diagnostic_images else None,
        "instance_rgb_path": str(rgb_path.resolve()) if diagnostic_images else None,
        "overlay_path": str(overlay_path.resolve()),
        "cellpose_mask_path": str(cellpose_mask_path.resolve()) if diagnostic_images else None,
        "cellpose_instance_rgb_path": str(cellpose_rgb_path.resolve()) if diagnostic_images else None,
        "cellpose_overlay_path": str(cellpose_overlay_path.resolve()) if diagnostic_images else None,
        "signal_path": str(signal_path.resolve()) if diagnostic_images else None,
        "support_path": str(support_path.resolve()) if diagnostic_images else None,
    ...
    metric["object_metrics_path"] = None
    if results_csv:
...
def make_largest_object_crops(rows: list[dict], crop_dir: Path) -> None:
    ...
    for row in rows:
        labels = full_metrics.load_labels(Path(row.get("mask_path") or row["working_mask_path"]))
...
def write_report(rows: list[dict], summary: dict, out_path: Path, output_options: dict) -> None:
    ...
    if output_options["results_csv"]:
        lines.append("- `results.csv`: endpoint metric table requested by the user.")
    if output_options["comparison_gallery"]:
        lines.append("- `comparison_gallery.png`: requested source/overlay/instance comparison gallery.")
    if output_options["statistics_figures"]:
        lines.append("- `summary_metrics.png`: requested morphology statistical figure.")
    if output_options["diagnostic_images"]:
        lines.extend(
            [
                "- `runs/<condition>/Dxx/*_mask_16bit.png`: requested 16-bit instance masks.",
                "- Requested Cellpose, instance, signal, and support diagnostic images are stored under `runs/`.",
            ]
        )
    if output_options["object_crops"]:
        lines.append("- `crops/*_largest_object_crop.png`: requested source/overlay crops.")
    lines.append("- `results.json` and per-image metric JSON files are machine-readable job metadata used by the application.")
    out_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    output_options = {
        "overlays": True,
        "report": True,
        "statistics_figures": bool(args.statistics_figures),
        "comparison_gallery": bool(args.comparison_gallery),
        "object_crops": bool(args.object_crops),
        "diagnostic_images": bool(args.diagnostic_images),
        "results_csv": bool(args.results_csv),
    }
    ...
    for idx, tif in enumerate(ordered_inputs, start=1):
        ...
        row = process_one(
            ...
            object_crops=args.object_crops,
    ...
    results = {
        ...
        "outputs": {
            "results_csv": str(results_csv.resolve()) if args.results_csv else None,
            "results_json": str(results_json.resolve()),
            "report_md": str(report_path.resolve()),
            "comparison_gallery": str(gallery_path.resolve()) if args.comparison_gallery else None,
            "summary_metrics": str(summary_figure_path.resolve()) if args.statistics_figures else None,
            "crops_dir": str((out_dir / "crops").resolve()) if args.object_crops else None,
    ...
    write_report(rows, summary, report_path, output_options)
